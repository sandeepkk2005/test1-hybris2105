...
public class DefaultBackofficeSearchservicesSystemSetupConfigTest
{
	...
	private static final String BACKOFFICE_SMART_SEARCH_ROOTS_FILE_ENCODING_KEY = "backoffice.search.services.roots.file.encoding";
	private static final String BACKOFFICE_SMART_SEARCH_ROOTS_LANGUAGE_SEPARATOR_KEY = "backoffice.search.services.roots.language.separator";
	...
	@InjectMocks
	private DefaultBackofficeSearchservicesSystemSetupConfig config;
	...
	public void shouldReturnConfiguredListOfPaths()
	{
		...
		//when
		final Collection<String> roots = config.readRoots(BACKOFFICE_SMART_SEARCH_LOCALIZED_ROOTS_KEY);
	...
	{
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_SEPARATOR_KEY)).thenReturn(SEMICOLON);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_LOCALIZED_ROOTS_KEY))
				.thenReturn(CONFIGURED_FILE_LIST);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_FILE_ENCODING_KEY))
				.thenReturn(FILE_ENCODING);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_LANGUAGE_SEPARATOR_KEY))
				.thenReturn(LANGUAGE_SEPARATOR);
	}

	private void mockConfigResolverWithEmptyKeys()
	{
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_SEPARATOR_KEY)).thenReturn(null);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_LOCALIZED_ROOTS_KEY)).thenReturn(null);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_FILE_ENCODING_KEY)).thenReturn(null);
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_ROOTS_LANGUAGE_SEPARATOR_KEY))
				.thenReturn(null);
	}
	...
	{
		when(configStringResolver.resolveConfigStringParameter(BACKOFFICE_SMART_SEARCH_NON_LOCALIZED_ROOTS_KEY))
				.thenReturn(CONFIGURED_FILE_LIST);
	}
}

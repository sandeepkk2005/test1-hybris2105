...
public final class WebhookEventToItemSender implements WebhookEventSender
{
	                                ...
	                                @NotNull final ModelService modelService)
	{
		...
		this.itemModelSearchService = new DefaultItemModelSearchService(modelService);
	}
	...
	private Optional<ItemModel> findItem(final AbstractEvent event)
	{
		...
		{
			return itemModelSearchService.nonCachingFindByPk(((WebhookEvent) event).getPk());
		}
	...
	private void sendItem(final ItemModel item, final EventSourceData eventSourceData)
	{
		...
		webhookConfigs.stream()
		              .filter(config -> isMatchingDestinationTarget(config, eventSourceData))
		              .forEach(config -> filterAndSend(config, item, eventType));
	}

	private boolean isMatchingDestinationTarget(final WebhookConfigurationModel webhookConfiguration,
	                                            final EventSourceData eventSourceData)
	{
		return webhookConfiguration.getDestination().getDestinationTarget().getId()
		                           .contentEquals(eventSourceData.getEventConfig().getDestinationTarget().getId());
	}
	...
	private void filterAndSend(final WebhookConfigurationModel config, final ItemModel item, final EventType eventType)
	{
		...
		filterService.filter(item, config.getFilterLocation())
		             .ifPresentOrElse(it -> sendToWebhook(config, it, eventType),
	...
	private void logInfo(final ItemModel item, final WebhookConfigurationModel cfg)
	{
		...
		{
			LOGGER.info("Sent item of type '{}' to {}", item.getItemtype(), ToStringUtil.toString(cfg));
		}
	}

...
public class DefaultB2BCommerceUserServiceTest
{
	...
	@Test
	public void testShouldDisableLoginTooWhenDisableCustomer()
	{
		Assert.assertTrue(user.getActive().booleanValue());

		defaultB2BCommerceUserService.disableCustomer(USER_ID);

		Assert.assertTrue("User login was not disabled", user.isLoginDisabled());
	}
	...
	@Test
	public void testShouldEnableLoginWhenEnableCustomer()
	{
		user.setActive(Boolean.FALSE);

		defaultB2BCommerceUserService.enableCustomer(USER_ID);

		Assert.assertFalse("User login was not enabled", user.isLoginDisabled());
	}

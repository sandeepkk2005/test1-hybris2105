...
class ODataItemTypeMatchIntegrationTest extends ServicelayerSpockSpecification {
    ...
    private static final String TEST_NAME = "ODataItemTypeMatch"
    ...
    private static final String IO2 = "${TEST_NAME}_IO2"
    ...
    def "post where abstract attribute"() {
        ...
        final ODataRequestBuilder oDataRequest =
                postRequestWithEntitySet("Categories")
                        .withBody(
                                json()
                                        .withField(CODE, SUPERTYPE_ITEM_KEY)
                                        .withField("allowedPrincipals", [json().withField("uid", user)])
                        )
        ...
        IntegrationTestUtil.removeSafely UserModel, { it.uid == user }
    }

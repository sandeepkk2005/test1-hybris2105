...
class OrgCustomerManagementControllerTest extends AbstractUserTest {
	...
	def "B2B Admin disabling organizational customer also disables login #orgCustomerId"() {
		given: "a registered and logged in customer with B2B Admin role"
		def b2bAdminCustomer = [
				'id'      : B2BADMIN_USERNAME,
				'password': B2BADMIN_PASSWORD
		...
		when: "he requests to update b2bCustomer password and disable account"
		restClient.patch(
				path: getBasePathWithSite() + '/users/' + b2bAdminCustomer.id + ORG_CUSTOMERS_PATH + "/" + orgCustomerId,
				body: patchBodySetPasswordAndInactive,
				contentType: JSON,
				requestContentType: JSON)


		and: "b2bCustomer details are updated and is disabled"
		...
		then: "b2bCustomer can not login"
		def responseData = getOAuth2TokenUsingPassword(restClient, getClientId(), getClientSecret(), email, password, false);
		assert responseData.error == 'invalid_grant'
		assert responseData.error_description == 'User is disabled'

		where:
		orgCustomerId                          | email                          | password | patchBodySetPasswordAndInactive
		"6a2a41a3-c54c-4ce8-a2d2-0324e1c32a24" | "fabian.sanchez@rustic-hw.com" | "123456" | ["password": "123456", "active": false]
	}

	def "B2B Admin enabling organizational customer also enables login #orgCustomerId"() {
		given: "a registered and logged in customer with B2B Admin role"
		def b2bAdminCustomer = [
				'id'      : B2BADMIN_USERNAME,
				'password': B2BADMIN_PASSWORD
		...
		when: "he requests to update b2bCustomer password and set account as active"
		restClient.patch(
				path: getBasePathWithSite() + '/users/' + b2bAdminCustomer.id + ORG_CUSTOMERS_PATH + "/" + orgCustomerId,
				body: patchBodySetPasswordAndActive,
				contentType: JSON,
				requestContentType: JSON)

		and: "b2bCustomer details are updated and is active customer"
		...
		then: "b2bCustomer can login"

		def responseData = getOAuth2TokenUsingPassword(restClient, getClientId(), getClientSecret(), email, password, false);
		assert responseData.token_type == 'bearer'
		assert responseData.access_token
		assert responseData.expires_in
		assert responseData.refresh_token

		where:
		orgCustomerId                          | email                          | password | patchBodySetPasswordAndActive
		"6a2a41a3-c54c-4ce8-a2d2-0324e1c32a24" | "fabian.sanchez@rustic-hw.com" | "123456" | ["password": "123456", "active": true]
	}
	...
	def "B2B Admin changes organizational customer password #orgCustomerId"() {
		...
		"6a2a41a3-c54c-4ce8-a2d2-0324e1c32a24" | "fabian.sanchez@rustic-hw.com" | "123456" | ["password": "123456"]
	}

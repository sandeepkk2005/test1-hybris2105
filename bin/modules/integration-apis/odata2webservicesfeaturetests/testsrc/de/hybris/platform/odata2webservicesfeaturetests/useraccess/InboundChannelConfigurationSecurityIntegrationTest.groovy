...
class InboundChannelConfigurationSecurityIntegrationTest extends ServicelayerSpockSpecification {
    ...
    private static final String TEST_NAME = "InboundChannelConfigurationSecurity"
    private static final String USER_UID = "${TEST_NAME}_User1"
    private static final String INTEGRATION_ADMIN_UID = "${TEST_NAME}_IntegrationAdmin_User"
    private static final String CLIENT_ID = "${TEST_NAME}_Cred1"
    ...
    def setupSpec() {
        ...
        givenUserExistsWithUidAndGroups(INTEGRATION_ADMIN_UID, PASSWORD, "integrationadmingroup")
    ...
    def cleanup() {
        removeAllPermissions(CatalogModel._TYPECODE, INTEGRATION_ADMIN_UID)
    ...
    def cleanupSpec() {
        ...
        IntegrationTestUtil.findAny(EmployeeModel, { it.uid == INTEGRATION_ADMIN_UID || it.uid == USER_UID }).ifPresent {
    ...
    def "BasicAuth #method request will result in a 404 when the IntegrationObject is not found"() {
        ...
        def response = basicAuthRequest()
                .path(name)
                .path(CATALOGS)
                .credentials(INTEGRATION_ADMIN_UID, PASSWORD)
                .build()
                .method(method)
    ...
    def "Basic Auth #method requests are authenticated when a BASIC InboundChannel exists for the ChannelConfigTestIO"() {
        ...
        inboundServicesConfiguration.setLegacySecurity(isLegacySecurity)
        and:
        addPermission(CatalogModel._TYPECODE, PermissionsConstants.READ, INTEGRATION_ADMIN_UID)
        addPermission(CatalogModel._TYPECODE, PermissionsConstants.REMOVE, INTEGRATION_ADMIN_UID)
    ...
    def "Basic Auth #method persistence requests are authenticated when a BASIC InboundChannel exists for the ChannelConfigTestIO"() {
        ...
        inboundServicesConfiguration.setLegacySecurity(isLegacySecurity)
        and:
        addPermission(CatalogModel._TYPECODE, PermissionsConstants.CREATE, INTEGRATION_ADMIN_UID)
        addPermission(CatalogModel._TYPECODE, PermissionsConstants.CHANGE, INTEGRATION_ADMIN_UID)
    ...
    @Test
    @Unroll
    def "Basic Auth #method requests are denied when legacy security is disabled false and no InboundChannelConfiguration exists for the IO"() {
        given:
        inboundServicesConfiguration.setLegacySecurity(false)
        and:
        inboundChannel.cleanup()
    ...
    def "Oauth2 password #method request is authenticated with OAuthClientDetails when an OAUTH InboundChannel exists for the IO"() {
        ...
        inboundChannelConfigurationExistsWithAuthenticationType("OAUTH")
        and:
        def oAuthClientId = "${TEST_NAME}_someOtherClientId"
        ...
        def response = oAuth2RequestWithPasswordGrantType()
                ...
                .resourceOwner(INTEGRATION_ADMIN_UID, PASSWORD)
                .path(IO)
                .path("$CATALOGS('$CATALOG_ID')")
                .build()
                .accept(MediaType.APPLICATION_JSON)
        ...
        cleanup:
        IntegrationTestUtil.remove OAuthClientDetailsModel, {it.clientId == oAuthClientId }

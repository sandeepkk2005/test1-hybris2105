...
public class IntegrationTestUtil extends ServicelayerTestLogic
{
	...
	/**
	 * Imports impex file located on the classpath into the platform
	 *
	 * @param path path of the IMPEX file resource on the classpath.
	 * @throws ImpExException when import failed. Most likely the IMPEX is invalid.
	 */
	public static void importClasspathImpEx(final String path) throws ImpExException
	{
		importCsv(path, StandardCharsets.UTF_8.name());
	}
	...
	{
		importImpEx(Arrays.asList(lines));
	}
	 ...
	 * @throws ImpExException when import failed. Most likely the IMPEX is invalid.
	 */
	public static void importImpEx(final Collection<String> lines) throws ImpExException
	{
	...
	{
		LOG.debug("Deleting all {} items", type.getSimpleName());
	...
	/**
	 * Removes specified model items.
	 *
	 * @param items items to be removed.
	 */
	public static void removeAll(final Collection<? extends ItemModel> items)
	{
		try
		{
	...
	public static <T> void remove(final T model)
	{
		try
		{
			...
			LOG.debug("{} has been deleted already", model, e);
		}
	}
	 ...
	 *
	 * @param type      type of the items to be deleted.
	 * @param predicate condition, the items to be deleted should match.
	 * @param <T>       type of the items being deleted
	 */

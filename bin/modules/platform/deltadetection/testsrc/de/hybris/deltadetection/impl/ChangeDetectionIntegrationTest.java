...
public class ChangeDetectionIntegrationTest extends ServicelayerBaseTest
{
	...
	public void testFindChangesForModifiedItem() throws Exception
	{
		...
		assertThat(change.getVersion()).isEqualTo(testCustomerJan.getModifiedtime());
		assertThat(markerVersion.before(change.getVersion())).isTrue();
	...
	public void testFindChangesForRemovedItems() throws Exception
	{
		...
		assertThat(changes).extracting("itemPK").containsOnly(oldPkJan.getLong(), oldPkPiotr.getLong(), oldPkTitleFoo.getLong());
	}
	...
	public void testConsumeChangesForDifferentChangeTypesAndDifferentItemTypes() throws Exception
	{
		...
		saveVersionMarker(testTitleBar.getPk(), testTitleBar.getModifiedtime(),
				...
				composedTypeTitle, STREAM_ID_XXX);
		// bar is up to date
		final ItemChangeDTO changesJan;
		final ItemChangeDTO changesPiotr;
		final ItemChangeDTO changesFoo;
		final ItemChangeDTO changesBar;
	...
	public void testFindChangesByType() throws Exception
	{
		...
		assertThat(changes).extracting("itemPK").containsOnly(oldPkTitleFoo.getLong(), testTitleBar.getPk().getLong());
		assertThat(changes).extracting("itemComposedType").containsOnly("Title");
	}
	...
	public void testFindChangesForModifiedItemStreamAware() throws Exception
	{
		...
		assertThat(markerVersion.before(changeXXX.getVersion())).isTrue();
	...
	public void testFindChangesByTypeStreamAware() throws Exception
	{
		...
		assertThat(changesXXX).extracting("itemPK").containsOnly(oldPkTitleFoo.getLong(), testTitleBar.getPk().getLong());
		assertThat(changesXXX).extracting("itemComposedType").containsOnly("Title");
		assertThat(changesXXX).extracting("streamId").containsOnly(STREAM_ID_XXX);
		...
		assertThat(changesYYY).extracting("itemPK").containsOnly(testTitleBar.getPk().getLong());
		assertThat(changesYYY).extracting("itemComposedType").containsOnly("Title");
		assertThat(changesYYY).extracting("streamId").containsOnly(STREAM_ID_YYY);
	}

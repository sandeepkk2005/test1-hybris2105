...
public class Y2YSyncMasterJobPerformableTest extends ServicelayerBaseTest
{
	...
	public void setUp() throws Exception
	{
		...
		for (final CatalogVersionModel ctgVer : vers)
		{
			final Y2YStreamConfigurationModel ctgVerConf = syncConfigService.createStreamConfiguration(zipSyncContainer,
					...
					prepareCatalogVersionAttributeDescriptors(),
					Collections.emptySet());
			final Y2YStreamConfigurationModel categConf = syncConfigService.createStreamConfiguration(zipSyncContainer,
					"Category",
					ctgVer,
					prepareCategoryAttributeDescriptors(),
					Collections.emptySet());
			final Y2YStreamConfigurationModel mediaConf = syncConfigService.createStreamConfiguration(zipSyncContainer, "Media",
					...
					Sets.newHashSet(
							syncConfigService.createUntypedColumnDefinition(
	...
	public void shouldRunSyncProcessToZip() throws Exception
	{
		...
		assertThat(foundMediaData).isNotNull().extracting(MediaModel::getCode).isEqualTo("data-" + cronJob.getCode());

		assertThat(foundMediaBinaries).isNotNull().extracting(MediaModel::getCode).isEqualTo("media-" + cronJob.getCode());
	...
	private CatalogUnawareMediaModel getImportMedia(final Y2YSyncCronJobModel cronJob,
	                                                final Function<Y2YSyncCronJobModel, CatalogUnawareMediaModel> cronJobCallback)

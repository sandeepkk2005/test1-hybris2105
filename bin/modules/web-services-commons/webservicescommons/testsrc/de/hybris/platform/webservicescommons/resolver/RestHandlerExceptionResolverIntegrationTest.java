...
public class RestHandlerExceptionResolverIntegrationTest extends ServicelayerBaseTest
{
	...
	@Test
	public void shouldHaveDefaultOrderSetOnRestExceptionResolver()
	{
		final RestExceptionResolver bean = Registry.getApplicationContext()
				.getBean("testRestExceptionResolver", RestExceptionResolver.class);
		Assert.assertEquals(0, bean.getOrder());
	}

	@Test
	public void shouldHaveOrderSetOnRestExceptionResolver()
	{
		final RestExceptionResolver bean = Registry.getApplicationContext()
				.getBean("testRestExceptionResolverOrderTen", RestExceptionResolver.class);
		Assert.assertEquals(10, bean.getOrder());
	}
}

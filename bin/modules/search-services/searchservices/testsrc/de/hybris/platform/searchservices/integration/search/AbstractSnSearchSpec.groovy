...
abstract class AbstractSnSearchSpec extends AbstractSnIntegrationSpec {
	...
	def setupSpec() {
		...
		objectMapper.addMixIn(AbstractSnExpressionQuery, SnExpressionQueryMixin)
	...
	protected final List<SnQuerySpec> loadQuerySpecJson(String resource, Set<String> skipTestIds = []) {
		...
		return objectMapper.readValue(url, type).findAll { skipTestIds.contains(it.testId) == false }
	}
	...
	@JsonSubTypes([
		...
		@JsonSubTypes.Type(value = SnExistsQuery, name = SnExistsQuery.TYPE)
	])
	static class SnQueryMixin {
	}

	static class SnExpressionQueryMixin {

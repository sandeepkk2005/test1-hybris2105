...
public class OrderNotificationEmailContext extends ChineseAbstractEmailContext<OrderProcessModel>
{
	...
	protected void updateDeliveryAddress(final OrderModel orderModel)
	{
		...
		if (Objects.nonNull(regionModel) && Objects.nonNull(regionData))
		{
			regionData.setName(regionModel.getName(emailLocale));
		}

		if (Objects.nonNull(cityModel))
		{
			String cityName = cityModel.getName(emailLocale);
			addressData.setTown(cityName);
			if(Objects.nonNull(cityData)) {
				cityData.setName(cityName);
			}
		}

		if (Objects.nonNull(districtModel) && Objects.nonNull(districtData))
		{

...
class SopTest extends AbstractCartTest {
	...
	def "Get SOP payment request details : #format"() {
		...
		def val = createCustomerWithCartForIntegrationSite(format, withBillingEmail);
		def customer = val[0]
		def cart = val[1]
		...
		with(response) {
			...
			resp.find { it.key == 'billTo_email' && it.value == (withBillingEmail ? CUSTOMER_EMAIL : customer.id) }
	...
	def "Guest places oder using SOP : #format"() {
		...
		setAddressForAnonymousCart(restClient, GOOD_ADDRESS_DE_JSON, cart.guid, format, getBasePathWithIntegrationSite())
	 ...
	 * @param withBillingEmail when true sets email as part of billing address, this email should be used as billing email in payment process, when false(default), customer id(email) is used.
	 * @return created customer and cart
	 */
	protected createCustomerWithCartForIntegrationSite(format, withBillingEmail = false) {
		...
		def address = createAddressWithOptions(restClient, customer, format: format, basePathWithSite: getBasePathWithIntegrationSite(), withBillingEmail: withBillingEmail)

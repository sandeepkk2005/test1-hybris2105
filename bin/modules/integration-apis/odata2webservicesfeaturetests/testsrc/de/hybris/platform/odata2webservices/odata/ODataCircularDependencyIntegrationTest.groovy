...
class ODataCircularDependencyIntegrationTest extends ServicelayerSpockSpecification {
    ...
    private static final def TEST_NAME = "ODataCircularDependency"
    private static final def CODE = "${TEST_NAME}_Order1"
    private static final def OTHER_CODE = "${TEST_NAME}_Order2"
    private static final def IO_CODE = "${TEST_NAME}_IO"
    private static final def CURRENCY = 'EUR'
    private static final def USER_ID = "${TEST_NAME}_john"
    private static final def PRODUCT = 'jewels'
    private static final def CATALOG_ID = "${TEST_NAME}_Jewelry"
    private static final def VERSION = 'Demo'
    private static final def UNIT = "${TEST_NAME}_Unit"
    ...
    def cleanup() {
        IntegrationTestUtil.remove OrderModel, {it.code == CODE || it.code == OTHER_CODE }
    }
    ...
    def "successfully creates an item with a missing nested required non-key reference attribute of the same type"() {
        ...
        def parentItemCode = "${TEST_NAME}_parentCode"
        def childItemCode = "${TEST_NAME}_childCode"
        IntegrationTestUtil.importImpEx(
                'INSERT_UPDATE IntegrationObjectItem; integrationObject(code)[unique = true]; code[unique = true]; type(code)',
        ...
        cleanup:
        IntegrationTestUtil.remove TestIntegrationItemModel, { it.code == parentItemCode }
        IntegrationTestUtil.remove TestIntegrationItem3Model, { it.code == childItemCode }
    }

...
@Unroll
@IntegrationTest
public class SnSearchFacetsRangeBucketsFacetSpec extends AbstractSnSearchDataDrivenSpec {
	...
	@Test
	def "Search with range buckets facets #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		if (row.languages) {
		...
		where:
		row << loadSearchSpecJson("/searchservices/test/integration/search/facets/validRangeBucketsFacets.json")
	}

	@Test
	def "Fail to search with range buckets facets #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		if (row.languages) {
			applyLanguages(row.languages)
		}
		...
		where:
		row << loadSearchSpecJson("/searchservices/test/integration/search/facets/invalidRangeBucketsFacets.json")
	}
}

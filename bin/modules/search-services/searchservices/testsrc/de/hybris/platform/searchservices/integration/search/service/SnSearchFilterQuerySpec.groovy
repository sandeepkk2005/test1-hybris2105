...
class SnSearchFilterQuerySpec extends AbstractSnSearchDataDrivenSpec {
	...
	@Test
	def "Add filter with localized/qualified query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		if (row.languages) {
	...
	@Test
	def "Add filter with invalid localized/qualified query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		if (row.languages) {
			applyLanguages(row.languages)
		}
	...
	@Test
	def "Add filter with exists query #row.testId: description=#row.description, data=#row.data"(row) {
		when:
		SnSearchResult searchResult = executeSearchQuery(INDEX_TYPE_ID, this.&createSearchQuery, this.&addDefaultSearchQuerySort, { SnSearchQuery searchQuery ->
			SnFilter filter = new SnFilter()
			filter.setQuery(row.data)
	...
	@Test
	def "Add filter with invalid exists query #row.testId: description=#row.description, data=#row.data"(row) {
		when:
		SnSearchResult searchResult = executeSearchQuery(INDEX_TYPE_ID, this.&createSearchQuery, this.&addDefaultSearchQuerySort, { SnSearchQuery searchQuery ->
			SnFilter filter = new SnFilter()
			filter.setQuery(row.data)

...
@IntegrationTest
class IntegrationObjectDefinitionIntegrationTest extends ServicelayerSpockSpecification {
    @Resource(name = "integrationObjectService")
    private IntegrationObjectService ioService

    private static final String TEST_NAME = "IntegrationObjectDefinition"
    ...
    def "uniqueness of integrationObjectItem is determined only by code"() {
        ...
        IntegrationTestUtil.importImpEx(
                'INSERT_UPDATE IntegrationObjectItem; integrationObject(code)[unique = true]; code[unique = true]; type(code)',
                "                                          ; $IO                                   ; Product            ; CatalogVersion")

        when:
        def ioItemType = ioService.findItemTypeCode("$IO", "Product")

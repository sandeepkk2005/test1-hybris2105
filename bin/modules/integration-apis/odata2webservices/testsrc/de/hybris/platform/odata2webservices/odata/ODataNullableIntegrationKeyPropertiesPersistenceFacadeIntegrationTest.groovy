...
@IntegrationTest
class ODataNullableIntegrationKeyPropertiesPersistenceFacadeIntegrationTest extends ServicelayerSpockSpecification {
    def TEST_NAME = "ODataNullableIntegrationKeyPropertiesPersistenceFacade"
    def ORDER_IO = "${TEST_NAME}_OrderEntryExample"
    def PRODUCT_IO = "${TEST_NAME}_TestForProductFeatureIO"
    //JsonBuilder complains when using String replacement. Using concat instead.
    def CATALOG = TEST_NAME.concat("_Default")
    def CATALOG_VERSION = "${CATALOG}:Staged"
    ...
    def catalogJson() {
        JsonBuilder.json().withId(CATALOG)
    }

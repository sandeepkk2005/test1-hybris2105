...
class RushInstallManager extends BaseInstallManager_1.BaseInstallManager {
    ...
    canSkipInstall(lastModifiedDate) {
        console.log(os.EOL +
            safe_1.default.bold(`Checking ${RushConstants_1.RushConstants.nodeModulesFolderName} in ${this.rushConfiguration.commonTempFolder}`) +
            os.EOL);
    ...
    _fixupNpm5Regression() {
        ...
        if (anyChanges) {
            console.log(os.EOL + safe_1.default.yellow(Utilities_1.Utilities.wrapWords(`Applied workaround for NPM 5 bug`)) + os.EOL);
        }
    }
    /**
    ...
    _findOrphanedTempProjects(shrinkwrapFile) {
        ...
        for (const tempProjectName of shrinkwrapFile.getTempProjectNames()) {
            if (!this.rushConfiguration.findProjectByTempName(tempProjectName)) {
                console.log(os.EOL +
                    safe_1.default.yellow(Utilities_1.Utilities.wrapWords(`Your ${this.rushConfiguration.shrinkwrapFilePhrase} references a project "${tempProjectName}" which no longer exists.`)) +
                    os.EOL);
    ...
    _findMissingTempProjects(shrinkwrapFile) {
        ...
        for (const rushProject of this.rushConfiguration.projects) {
            if (!tempProjectNames.has(rushProject.tempProjectName)) {
                console.log(os.EOL +
                    safe_1.default.yellow(Utilities_1.Utilities.wrapWords(`Your ${this.rushConfiguration.shrinkwrapFilePhrase} is missing the project "${rushProject.packageName}".`)) +
                    os.EOL);
                return true; // found one
            }
        }

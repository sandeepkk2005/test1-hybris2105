...
export class DataService {
	...
	_buildUrl(rootUrl) {
		...
		url = rootUrl + `/${encodeURIComponent(this.tenant)}/strategies/${encodeURIComponent(this.strategy)}/products`;

		let queryString = '';
		if (this.category) {
			queryString = this._buildQueryString(queryString, `category=${encodeURIComponent(this.category)}`);
		}

		if (this.facets) {
			queryString = this._buildQueryString(queryString, `facets=${encodeURIComponent(this.facets)}`);
		}

		if (this.numberToDisplay) {
			queryString = this._buildQueryString(queryString, `pageSize=${encodeURIComponent(this.numberToDisplay)}`);
		}

		if(this.siteId) {
			queryString = this._buildQueryString(queryString, `site=${encodeURIComponent(this.siteId)}`);
		}

		if(this.language) {
			queryString = this._buildQueryString(queryString, `language=${encodeURIComponent(this.language)}`);
		}

		if(this.productId) {
			queryString = this._buildQueryString(queryString, `products=${encodeURIComponent(this.productId)}`);
		}

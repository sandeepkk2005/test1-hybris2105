...
import ydocumentcartpackage.persistence.polyglot.repository.documentcart.cart.DataSourceFactory;
...
public class YDocumentCartRepositoryTest extends ServicelayerBaseTest
{
	...
	@Resource
	private DataSourceFactory cartDataSourceFactory;
	...
	public void shouldSaveCartWithEntryGroups()
	{
		...
		final CartEntryModel cartEntry1 = cartModelsCreator.createCartEntry(unit, product, cart, 1L);
		cartEntry1.setBasePrice(0.5d);
		cartEntry1.setEntryGroupNumbers(Set.of(1));
		final CartEntryModel cartEntry2 = cartModelsCreator.createCartEntry(unit, product, cart, 4L);
		cartEntry2.setBasePrice(1.1d);
		cartEntry2.setEntryGroupNumbers(Set.of(2));
		final CartEntryModel cartEntry3 = cartModelsCreator.createCartEntry(unit, product, cart, 4L);
		cartEntry3.setBasePrice(1d);
		cartEntry3.setEntryGroupNumbers(Set.of(1, 2));
	...
	{
		//SQL Server only
		Assume.assumeThat(cartDataSourceFactory.getDatabaseName(), Matchers.equalTo(Config.DatabaseName.SQLSERVER.getName()));

		final int entriesCount = 250;

		final CartModel cart;
		//cache changes to make test faster
		try (final StorageCache.CacheContext cacheContext = cachedDocumentStorage.initCacheContext())
		{
			cart = cartModelsCreator.createCart(CartModel.DESCRIPTION, 1d);

			for (int i = 0; i < entriesCount; i++)
			{
				cartModelsCreator.createCartEntry(unit, product, cart, 1L);
			}
		...
		final Identity cartIdentity = PolyglotPersistence.identityFromLong(cart.getPk().getLong());
		final ItemState persistedCart = repository.get(cartIdentity);

		assertThat(persistedCart).isNotNull();

		modelService.refresh(cart);

		assertThat(cart.getEntries()).hasSize(entriesCount);
	}

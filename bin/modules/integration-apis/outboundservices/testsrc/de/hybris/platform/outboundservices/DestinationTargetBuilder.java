 ...
 */
public class DestinationTargetBuilder extends ExternalResource
{
	private static final String DEFAULT_ID = "stoutoutboundtest";

	private final Set<String> createdDestinations;
	...
	{
		final String derivedId = deriveId();
		IntegrationTestUtil.importImpEx(
				"INSERT_UPDATE DestinationTarget; id[unique = true] ; destinationChannel(code); registrationStatus(code)",
				"                               ; " + derivedId + " ; " + deriveChannel() + " ; " + deriveStatus());
		createdDestinations.add(derivedId);
		return getDestinationTargetById(derivedId).orElse(null);
	}
	 ...
	 */
	public void cleanup() {
		createdDestinations.forEach(this::deleteTargetById);
		createdDestinations.clear();
	}

	private void deleteTargetById(final String id)
	{
		IntegrationTestUtil.remove(DestinationTargetModel.class, m -> m.getId().equals(id));
	}
}

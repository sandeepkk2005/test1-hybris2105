 ...
 * the items changes.
 * @deprecated use {@link OutboundSyncStreamConfigurationBuilder} and {@link OutboundChannelConfigurationBuilder} instead.
 */
...
public class TestItemChangeDetector extends ExternalResource
{
	...
	private final Collection<OutboundSyncStreamConfigurationModel> allChangeStreams = new ArrayList<>();
	...
	public OutboundSyncStreamConfigurationModel createChangeStream(final String channelCode, final String typecode,
			...
			throws ImpExException
	{
		final OutboundSyncStreamConfigurationModel stream = persistStream(typecode, channelCode, filter);
		allChangeStreams.add(stream);
		getDeltaDetect().consumeChanges(getChangesFromStream(stream));
		return stream;
	}

	private static OutboundSyncStreamConfigurationModel persistStream(final String typecode, final String channelCode, final String filter)
			throws ImpExException
	{

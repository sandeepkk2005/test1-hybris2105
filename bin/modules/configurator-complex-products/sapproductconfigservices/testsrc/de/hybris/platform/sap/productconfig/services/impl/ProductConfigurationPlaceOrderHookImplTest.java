...
public class ProductConfigurationPlaceOrderHookImplTest
{
	...
	public void testAfterPlaceOrderResumesOnFinalizeException() throws InvalidCartException
	{
		...
		verify(configurationAbstractOrderIntegrationStrategy, times(1)).finalizeCartEntry(entry2);
		verify(configurationAbstractOrderIntegrationStrategy, times(1)).prepareForOrderReplication(orderEntry);
		verify(configurationAbstractOrderIntegrationStrategy, times(1)).prepareForOrderReplication(orderEntry2);
	}

	@Test
	public void testAfterPlaceOrderResumesOnPrepareForOrderReplicationException() throws InvalidCartException
	{
		prepareEntryList(true);
		doThrow(new RuntimeException("Prepare for order replication not possible"))
				.when(configurationAbstractOrderIntegrationStrategy).prepareForOrderReplication(orderEntry);

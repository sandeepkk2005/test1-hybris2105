...
public class CsticTypeMapperImplTest
{
	...
	public void testMapCsticModelToData_useDeltaPricesDerivedFromProductCode()
	{
		...
		assertTrue("delta pcrice mapped incorrect", new BigDecimal("125.99").compareTo(valueData.getPrice().getValue()) == 0);
		assertEquals("EUR", valueData.getPrice().getCurrencyIso());
	}

	@Test
	public void testMapCsticModelToData_useDeltaPricesDefaultBehaviour()
	{
		final CsticModel csticModel = ConfigurationTestData.createRadioButtonCstic();
		final CsticValueModel valueModel = csticModel.getAssignableValues().get(0);
		preparePrices(valueModel);
		final CsticData csticData = typeMapper.mapCsticModelToData(csticModel, GROUP_NAME, null);
		final CsticValueData valueData = csticData.getDomainvalues().get(0);
		assertTrue("absolute value price mapped incorrect",
				new BigDecimal("345.77").compareTo(valueData.getPrice().getValue()) == 0);
		assertEquals("EUR", valueData.getPrice().getCurrencyIso());
	}

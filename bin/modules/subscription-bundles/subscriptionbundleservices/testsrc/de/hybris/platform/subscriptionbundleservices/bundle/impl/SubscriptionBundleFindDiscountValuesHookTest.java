...
/**
 * JUnit test suite for {@link SubscriptionBundleFindDiscountValuesHook}
 */
@UnitTest
public class SubscriptionBundleFindDiscountValuesHookTest
{
	...
	private SubscriptionBundleFindDiscountValuesHook bundlePriceHook;
	...
	public void setUp() throws Exception
	{
		...
		bundlePriceHook = new SubscriptionBundleFindDiscountValuesHook();
		bundlePriceHook.setBundleRuleService(bundleRuleService);
		bundlePriceHook.setCommercePriceService(commercePriceService);
		bundlePriceHook.setCommercePriceService(commercePriceService);
		bundlePriceHook.setSubscriptionProductService(subscriptionProductService);
		bundlePriceHook.setBundleTemplateService(bundleTemplateService);
		...
		given(bundleTemplateService.getBundleEntryGroup(masterEntry)).willReturn(entryGroup);
	}
	...
	public void testFindDiscountValuesNoPrice() throws CalculationException
	{
		...
		final List<DiscountValue> discounts = bundlePriceHook.findDiscountValues(childEntry);
	...
	public void testOneTimeChargePriceRuleWinsAgainstPlan()
	{
		...
		bundlePriceHook.reduceOneTimePrice(pricePlan, priceRule, discountValues, currency, childEntry);
	...
	public void testOneTimeChargePriceNoRule()
	{
		...
		final List<DiscountValue> discountValues = Lists.newArrayList();
		bundlePriceHook.reduceOneTimePrice(pricePlan, null, discountValues, currency, childEntry);
	...
	public void testOneTimeChargePriceNoPlan()
	{
		...
		final List<DiscountValue> discountValues = Lists.newArrayList();
		bundlePriceHook.reduceOneTimePrice(null, priceRule, discountValues, currency, childEntry);
	...
	{
		given(bundleTemplateService.getBundleEntryGroup(childEntry)).willReturn(new EntryGroup());
		given(childEntry.getEntryGroupNumbers()).willReturn(Collections.singleton(Integer.valueOf(1)));

...
@IntegrationTest
class IntegrationServiceVirtualAttributesIntegrationTest extends ServicelayerSpockSpecification {
    private static final String TEST_NAME = "IntegrationServiceVirtualAttributes"
    private static final String IOI_CODE = "${TEST_NAME}_MyProduct"
    private static final String PHONE_IO = "${TEST_NAME}_IO"
    private static final String SERVICE_NAME = 'IntegrationService'
    private static final String PRODUCT_WITH_VIRTUAL_ATTRIBUTES_IO = "${TEST_NAME}_ProductWithVirtualAttributesIO"
    ...
    VirtualAttributeBuilder virtualAttributeBuilder = VirtualAttributeBuilder.attribute()
            .withIntegrationObject(PHONE_IO)
            .withItem('Product')
            .withRetrievalDescriptor VirtualAttributeBuilder.logicDescriptor()
    ...
    def "GET IntegrationObjectItemVirtualAttributes returns virtual attributes for Product"() {
        ...
        when:
        ODataResponse response = facade.handleRequest getVirtualAttributesRequest()
    ...
    def "GET IntegrationObjectVirtualAttributeDescriptors returns the default type when descriptor type was not provided"() {
        ...
        virtualAttributeBuilder
                .withName(testVirtualAttributeName1)
                .withRetrievalDescriptor(virtualAttributeDescriptorWithNoType(CONTEXT_RETRIEVAL_DESCRIPTOR_CODE))
                .setup()

        when:
        ODataResponse response = facade.handleRequest getVirtualAttributesRequest(['$expand': 'retrievalDescriptor/type'])
    ...
    def "Virtual Attribute Descriptor can be expanded when making a GET request of IntegrationObjectItems"() {
        given:
        def virtualAttributeName = 'testVirtualAttribute'
        virtualAttributeForProduct(virtualAttributeName)
        and:
        def params = ['$filter': 'attributeName eq ' + "'$virtualAttributeName'", '$expand': 'retrievalDescriptor']

        when:
        ODataResponse response = facade.handleRequest getVirtualAttributesRequest(params)
    ...
    private ODataContext getVirtualAttributesRequest(Map<String, String> params = [:]) {
        oDataGetContext("IntegrationObjectItemVirtualAttributes", params)
    }

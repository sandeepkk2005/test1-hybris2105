...
public class DefaultDroolsRuleMetadataGeneratorTest extends AbstractGeneratorTest
{
	...
	public void testGenerateMetadata() throws Exception
	{
		...
		when(droolsStringUtils.validateVariableName("META_ID")).thenReturn("META_ID");
		when(droolsStringUtils.encodeMvelStringLiteral("META_VALUE")).thenReturn("META_VALUE");
		// when
	...
	public void testGenerateTwoMetadata() throws Exception
	{
		...
		when(droolsStringUtils.validateVariableName("META_ID1")).thenReturn("META_ID1");
		when(droolsStringUtils.encodeMvelStringLiteral("META_VALUE1")).thenReturn("META_VALUE1");
		when(droolsStringUtils.validateVariableName("META_ID2")).thenReturn("META_ID2");
		when(droolsStringUtils.encodeMvelStringLiteral("META_VALUE2")).thenReturn("META_VALUE2");
		// when
		final String generatedDroolsCode = defaultDroolsRuleMetadataGenerator.generateMetadata(droolsRuleGeneratorContext,
				INDENTATION);
		// then

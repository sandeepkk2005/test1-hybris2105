...
class ProductTests extends BaseWSTest {
	...
	static final NUMBER_OF_ALL_INDEXED_PRODUCTS = 45
	...
	void testSearchProductsAutoSuggestRequiredParameterMissing() {
		...
		assert error =~ /Required \w+ parameter/
	}
	...
	void testProductExtendedDetailsSpecificSJON() {
		def con = testUtil.getSecureConnection("/products/872912a?options=BASIC,DESCRIPTION,GALLERY,CATEGORIES,PROMOTIONS,STOCK,REVIEW,CLASSIFICATION,REFERENCES,PRICE", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedJSONSlurper(con, false, false)
	...
	@Test
	void testProductUsingFallbackLanguageJSON() {
		def con = testUtil.getSecureConnection("/products/" + PRODUCT_CODE + "?lang=de", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedJSONSlurper(con, false, false)

		assert response.name == PRODUCT_NAME
		assert response.code == PRODUCT_CODE
	}

	@Test
	void testProductUsingFallbackLanguageXML() {
		def con = testUtil.getSecureConnection("/products/" + PRODUCT_CODE + "?lang=de", 'GET', 'XML', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedXMLSlurper(con, false, false)

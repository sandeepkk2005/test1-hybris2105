...
import static de.hybris.platform.webhookservices.util.WebhookServicesEssentialData.webhookServicesEssentialData
...
class WebhookServiceSecurityIntegrationTest extends ServicelayerSpockSpecification {
    ...
    private static final String ESSENTIAL_DATA_TARGET_ID = "webhookServices"
    private static final String EVENT_TYPE = ItemSavedEvent.canonicalName
    private static final String DEFAULT_WEBHOOK_CONFIG_KEY = "${CONSUMED_DESTINATION_ID}|${ESSENTIAL_DATA_TARGET_ID}|${IO_CODE}|${EVENT_TYPE}"
    ...
    @Shared
    @ClassRule
    ModuleEssentialData odata2webservices = odata2WebservicesEssentialData().withDependencies()
    @Shared
    @ClassRule
    ModuleEssentialData webhookEssentialData = webhookServicesEssentialData().withDependencies()
    @Rule
    WebhookConfigurationBuilder webhookConfiguration = webhookConfiguration()

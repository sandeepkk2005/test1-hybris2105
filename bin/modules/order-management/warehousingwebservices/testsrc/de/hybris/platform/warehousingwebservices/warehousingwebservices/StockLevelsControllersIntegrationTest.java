...
public class StockLevelsControllersIntegrationTest extends BaseWarehousingWebservicesIntegrationTest
{
	...
	public void postCreateStockLevelAdjustment_WithReleaseDate()
	{
		...
		stockLevels_Montreal_Camera.setReleaseDate(cal.getTime());
		modelService.save(stockLevels_Montreal_Camera);
		//when
		final StockLevelAdjustmentWsDTO stockLevelAdjustmentWsDTO = createStockLevelAdjustmentWsDTO(INCREASE_REASON, 3,
				COMMENT_TEXT);
		...
		final StockLevelAdjustmentsWsDTO response = getWsSecuredRequestBuilder()
				...
				.grantResourceOwnerPasswordCredentials()
				.path(STOCKLEVELS + "/product/" + Products.CODE_CAMERA + "/warehouse/" + Warehouses.CODE_MONTREAL + "/adjustment")
				.queryParam("releaseDate", dateFormat.format(new Date())).build().accept(MediaType.APPLICATION_JSON)
				.post(Entity.entity(stockLevelAdjustmentsWsDTO, MediaType.APPLICATION_JSON), StockLevelAdjustmentsWsDTO.class);
		//then
		assertEquals(3L, response.getStockLevelAdjustments().iterator().next().getQuantity().longValue());
		assertEquals(INCREASE_REASON, response.getStockLevelAdjustments().iterator().next().getReason());
	...
	{
		//Given
		final String sampleWareHouse = "exceptionTesting";
		final WarehouseService mockWarehouseService = setMockServiceToFacade();
		doThrow(new AmbiguousIdentifierException("======= This is an Exception =====")).when(mockWarehouseService)
				.getWarehouseForCode(Mockito.eq(sampleWareHouse));

		// When
		final Response response = getWsSecuredRequestBuilder().resourceOwner(WAREHOUSE_AGENT_USERNAME, WAREHOUSE_AGENT_PASSWORD)
				.client(DEFAULT_CLIENT_ID, DEFAULT_CLIENT_SECRET).grantResourceOwnerPasswordCredentials()
				.path("stocklevels/warehouses/" + sampleWareHouse).queryParam("fields", DEFAULT_FIELDS)
				.queryParam("currentPage", DEFAULT_CURRENT_PAGE).queryParam("pageSize", DEFAULT_PAGE_SIZE).build()
				.accept(MediaType.APPLICATION_JSON).get();
		final ErrorWsDTO error = response.readEntity(ErrorListWsDTO.class).getErrors().get(0);

		// Then
		assertEquals(400, response.getStatus());
		assertEquals("The application has encountered an error", error.getMessage());
		assertEquals(null, error.getReason());
		assertEquals(null, error.getSubject());
		assertEquals(null, error.getSubjectType());
	}
	...
	public void getStockLevelWithUnknownException()
	{
		...
		final String sampleWareHouse = "exceptionTesting";
		final WarehouseService mockWarehouseService = setMockServiceToFacade();
		doThrow(new ArrayIndexOutOfBoundsException("======= This is an Exception =====")).when(mockWarehouseService)
				.getWarehouseForCode(Mockito.eq(sampleWareHouse));
	...
	public void getStockLevelWithForwardException()
	{
		//Given
		final String sampleWareHouse = "exceptionTesting";
		final String errorMessage = "======= This is an Exception =====";
		final WarehouseService mockWarehouseService = setMockServiceToFacade();
		doThrow(new ModelNotFoundException("======= This is an Exception =====")).when(mockWarehouseService)
				.getWarehouseForCode(Mockito.eq(sampleWareHouse));
	...
	private WarehouseService setMockServiceToFacade()
	{
		final DefaultWarehousingStockLevelFacade warehousingStockLevelFacade = Registry.getApplicationContext()
				.getBean(DefaultWarehousingStockLevelFacade.class);
		final WarehouseService mockService = Registry.getApplicationContext()
				.getBean("spyOnwarehouseService", WarehouseService.class);

		warehousingStockLevelFacade.setWarehouseService(mockService);
		return mockService;
	}
}

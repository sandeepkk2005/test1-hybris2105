...
class SnSearchQueryFunctionRankRuleQuerySpec extends AbstractSnSearchDataDrivenSpec {
	...
	@Test
	def "Add query function rank rule with localized/qualified query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		float weight = 2f
		...
		if (row.qualifiers) {
			applyQualifiers(row.qualifiers)
		}
		...
		where:
		row << loadQuerySpecJson("/searchservices/test/integration/search/queries/validLocalizedAndQualifiedQueries.json")
	}

	@Test
	def "Add query function rank rule with invalid localized/qualified query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		float weight = 2f
		...
		where:
		row << loadQuerySpecJson("/searchservices/test/integration/search/queries/invalidLocalizedAndQualifiedQueries.json", ["1", "2"] as Set)
	}
	...
	@Test
	def "Add query function rank rule with exists query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		float weight = 2f
		...
		where:
		row << loadQuerySpecJson("/searchservices/test/integration/search/queries/validExistsQueries.json")
	}

	@Test
	def "Add query function rank rule with invalid exists query #row.testId: description=#row.description, data=#row.data"(row) {
		given:
		float weight = 2f
		...
		where:
		row << loadQuerySpecJson("/searchservices/test/integration/search/queries/invalidExistsQueries.json")
	}
}

	...
	describe('proxy', () => {
		...
		describe('openEditAction', () => {
			...
			it('is called proper functions should be called', () => {
				...
				// then
				expect(modalService.open).toHaveBeenCalled();
				//TODO: private $$watchers on the $rootScope? use any to fix temporarily
				expect(($rootScope as any).$$watchers).toEqual(null);
			});
		});
	});
	...
	describe('modal', () => {
		describe('modalDeleteActionController', () => {
			...
			it('is instantiated scope  properly initialized', () => {
				//TODO: Factory need to refactor, use any to fix temporarily
				const deleteActionControllerFactory = ContextMenuDeleteActionControllerFactory(mockConfig) as any;

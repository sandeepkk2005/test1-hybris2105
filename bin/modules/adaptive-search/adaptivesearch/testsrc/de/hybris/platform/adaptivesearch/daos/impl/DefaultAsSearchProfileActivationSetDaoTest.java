...
public class DefaultAsSearchProfileActivationSetDaoTest extends ServicelayerTransactionalTest
{
	...
	private final static String CATALOG2_ID = "hwcatalog2";
	...
	{
		// given
		importCsv("/adaptivesearch/test/daos/defaultAsSearchProfileActivationSetDaoTest.impex", StandardCharsets.UTF_8.name());
		...
		// then
		assertNotNull(activationSets);
		assertEquals(3, activationSets.size());
	}
	...
	@Test
	public void findWithCatalogVersions() throws Exception
	{
		// given
		importCsv("/adaptivesearch/test/daos/defaultAsSearchProfileActivationSetDaoTest.impex", StandardCharsets.UTF_8.name());
		final CatalogVersionModel catalogVersion = catalogVersionService.getCatalogVersion(CATALOG_ID, VERSION_STAGED);
		final CatalogVersionModel catalogVersion2 = catalogVersionService.getCatalogVersion(CATALOG2_ID, VERSION_STAGED);

		// when
		final List<AsSearchProfileActivationSetModel> activationSetResults = asSearchProfileActivationSetDao
				.findSearchProfileActivationSetsByCatalogVersionsAndIndexType(Arrays.asList(catalogVersion,catalogVersion2), INDEX_TYPE);

		// then
		assertEquals(2, activationSetResults.size());
		final List<CatalogVersionModel> collect1 = activationSetResults.stream()
				.map(AsSearchProfileActivationSetModel::getCatalogVersion).collect(Collectors.toList());
		assertThat(collect1).containsExactlyInAnyOrder(catalogVersion, catalogVersion2);
		final List<AbstractAsSearchProfileModel> collect = activationSetResults.stream()
				.map(AsSearchProfileActivationSetModel::getSearchProfiles).flatMap(Collection::stream).collect(Collectors.toList());

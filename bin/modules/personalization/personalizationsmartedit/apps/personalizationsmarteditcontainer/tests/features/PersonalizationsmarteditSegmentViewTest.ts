...
describe('Test Personalizationsmartedit Segment View Module', () => {
	...
	describe('Component API', () => {
		...
		//TODO: do we need verify private method or member defined ? use any to fix temporarily
		...
		it('should have proper api when initialized without parameters', () => {
			...
			expect((segmentViewComponent as any).isScrollZoneVisible).toBeDefined();
			...
			expect((segmentViewComponent as any).segmentFilter).toBeDefined();
			...
			expect((segmentViewComponent as any).moreSegmentRequestProcessing).toBeDefined();
			expect(segmentViewComponent.addMoreSegmentItems).toBeDefined();
			expect(segmentViewComponent.isTopContainer).toBeDefined();
			expect((segmentViewComponent as any).isEmptyContainer).toBeDefined();
	...
	describe('addMoreSegmentItems', () => {
		...
		it('should set properties if called', () => {
			...
			// then
			expect(segmentViewComponent.segments as any).toEqual([mockSegment1, mockSegment2, mockSegment3]);
			expect(segmentViewComponent.segmentPagination.getCount()).toBe(3);  // Lucky, it has getter
			expect(segmentViewComponent.segmentPagination.getPage()).toBe(0);
			expect(segmentViewComponent.segmentPagination.getTotalCount()).toBe(3);
			expect(segmentViewComponent.segmentPagination.getTotalPages()).toBe(1);
		});

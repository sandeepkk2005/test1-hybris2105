...
public class ChangeDetectionResetStreamTest extends ServicelayerBaseTest
{
	...
	{
		// given
		final ItemVersionMarkerModel activeIvm = createVersionMarker(fooTitle, VersionMarkerTS.PAST,
				ItemVersionMarkerStatus.ACTIVE);
	...
	public void resetStreamEndToEndTest()
	{
		...
		assertThat(beforeResetCollector.getChanges()).extracting("changeType").containsOnly(ChangeType.NEW, ChangeType.MODIFIED);
		...
		assertThat(afterResetCollector.getChanges()).extracting("changeType").containsOnly(ChangeType.MODIFIED);
	}

...
public class ItemChangesProcessorIntegrationTest extends ServicelayerBaseTest
{
	...
	public void testRunSuccessfully() throws Exception
	{
		...
		final MediaBatchingCollector collector = findChangesWithBatchingCollector(titleComposedType, streamId);
		...
		final MediasForType mediasForType1 = MediasForType.builder()
		                                                  .withComposedTypeCode(titleComposedType.getCode())
		                                                  .withImpExHeader(IMPEX_HEADER_CORRECT)
		                                                  .withDataHubColumns(StringUtils.EMPTY)
		                                                  .withMediaPks(mediaPks)
		                                                  .build();
		...
		verifyChangeConsumption(testTitleFoo, true, streamId); //consumed
		verifyChangeConsumption(testTitleBar, true, streamId); //consumed
	...
	public void testWaitWhenNoMatchingNodeGroupGiven() throws Exception
	{
		...
		finally
		{
			...
			verifyChangeConsumption(testTitleFoo, false, streamId); //not consumed
			modelService.refresh(condition);
	...
	public void testRunWithErrorsInImpexHeader() throws Exception
	{
		...
		final MediasForType mediasForType1 = MediasForType.builder()
		                                                  .withComposedTypeCode(titleComposedType.getCode())
		                                                  .withImpExHeader(IMPEX_HEADER_ERROR)
		                                                  .withDataHubColumns(StringUtils.EMPTY)
		                                                  .withMediaPks(mediaPks)
		                                                  .build();
		...
		try
		{
			TestUtils.disableFileAnalyzer(
					"de.hybris.y2ysync.task.runner.ItemChangesProcessorIntegrationTest.testRunWithErrorsInImpexHeader()");
	...
	private MediaBatchingCollector findChangesWithBatchingCollector(final ComposedTypeModel composedType, final String streamId)
	{
		final MediaBatchingCollector collector = new MediaBatchingCollector("testDeltaMedia", 3, modelService, mediaService);
		collector.setId("1");
		changeDetectionService.collectChangesForType(composedType, streamId, collector);
		return collector;
	}

	private void verifyChangeConsumption(final ItemModel item, final boolean consumptionExpected, final String streamId)
	{
		final ItemChangeDTO change = changeDetectionService.getChangeForExistingItem(item, streamId);
